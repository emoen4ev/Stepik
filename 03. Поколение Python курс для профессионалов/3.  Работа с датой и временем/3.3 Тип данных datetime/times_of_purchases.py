...
Дополните приведенный ниже код, чтобы он вывел текст До полудня, если большее число покупок было совершено до полудня,
или текст После полудня в противном случае.
...
before_noon = list(filter(lambda h: h.hour < 12, times_of_purchases))
afternoon = list(filter(lambda h: h.hour >= 12, times_of_purchases))

result = (len(before_noon) > len(afternoon) and 'До полудня') or (1 and 'После полудня')
