...
Ключом в словаре является имя ученика, а значением — кортеж, первый элемент которого — время начала решения,
второй элемент — время окончания решения. Дополните приведенный ниже код, чтобы он вывел имя ученика,
...
# min_value = min(datetime.strptime(value[1], '%d.%m.%Y %H:%M:%S') - datetime.strptime(value[0], '%d.%m.%Y %H:%M:%S')
#                 for value in data.values())
#
# name = [key for key, value in data.items() if datetime.strptime(value[1], '%d.%m.%Y %H:%M:%S')
#         - datetime.strptime(value[0], '%d.%m.%Y %H:%M:%S') == min_value]

# print(*name)


the_best_student = min(data, key=lambda student_name: datetime.strptime(data[student_name][1], '%d.%m.%Y %H:%M:%S')
                                          - datetime.strptime(data[student_name][0], '%d.%m.%Y %H:%M:%S'))
