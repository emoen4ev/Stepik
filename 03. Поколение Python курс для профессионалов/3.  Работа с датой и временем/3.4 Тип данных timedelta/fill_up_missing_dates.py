...
Тогда вызов функции: fill_up_missing_dates(dates)
...
def fill_up_missing_dates(data: list) -> list:
    pattern = '%d.%m.%Y'
    data = sorted([datetime.strptime(item, pattern) for item in data])
    diff = (data[-1] - data[0]).days

    data = [(data[0] + timedelta(days=i)).strftime(pattern) for i in range(diff + 1)]
